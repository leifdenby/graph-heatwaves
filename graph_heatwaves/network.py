...
def train(model, opt, trainLoader, nNodes, scheduler, nClass, lossFn, weights):
    ...
    for _batch in trainLoader:
        ...
        _recall[torch.isnan(_recall)] = 0.0
        _prec[torch.isnan(_prec)] = 0.0
        _f1[torch.isnan(_f1)] = 0.0
    ...
    return (
        np.mean(_lossList),
        np.mean(_accuList),
        [np.mean(_recallList), np.mean(_precList), np.mean(_f1List)],
    )
...
def oneStepEval(model, validLoader, nNodes, nClass, lossFn, weights):
    ...
    for _batch in validLoader:
        ...
        print(
            f"Max recall {torch.max(_recall)*100:1.2f}% at station {torch.argmax(_recall).item()}. Min TPR {torch.min(_recall)*100:1.2f}% at station {torch.argmin(_recall).item()}."
        )
